...
import { DS_MON_AN, type MonAn } from '../../data/monAnHue'
...
export default function MonAnYeuThich() {
  const [dsMon, setDsMon] = useState<MonAn[]>(DS_MON_AN.slice(0, 5))
  ...
  function handleDrop(e: React.DragEvent<HTMLLIElement>, idDich: string) {
    e.preventDefault()
  ...
  function handleKeyDown(e: React.KeyboardEvent<HTMLLIElement>, viTri: number) {
    if (!e.altKey) return
  ...
  function handleXoa(id: string) {
    ...
    if (mon) setThongBao('Đã xoá ' + mon.ten + ' khỏi danh sách')
  }
